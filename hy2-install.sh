...
while true; do
    ...
    if [ -z "$port" ]; then
      ...
      echo "$(random_color '必须是输入数字！请重新输入端口号：')"
      continue
    fi
  ...
  exit 1
    ...
    if [ -z "$port_jump" ]; then
     ...
     if [ "$start_port" -lt "$end_port" ]; then
        echo "$(random_color '请配置防火墙策略，开启端口跳跃功能，将范围重定向到主端口：')" "$port"
        break
      else
